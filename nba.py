...
def format_from_now_name(raw_text):
    ...
    for pattern, func in patterns:
        ...
        if match:
            ...
            break
    teams = re.findall(r'(?:[A-Z][a-z0-9]+|[A-Z]{2,}|[0-9]+[A-Za-z]+)(?:\s(?:[A-Z][a-z0-9]+|[A-Z]{2,}|[0-9]+[A-Za-z]+))*', raw_text)
    if len(teams) >= 2:
        return f"{time_str}  {teams[0]}  🆚  {teams[1]}"
    return f"{time_str}  {raw_text.strip()}"

# ===== 改写 fetch_home_matches() 使用 Playwright 抓取动态渲染 =====
...
def fetch_sportsbest_url(match_url):
    with sync_playwright() as p:
        ...
        while time.time() - start_ts < 5:
            html = page.content()
            soup = BeautifulSoup(html, "html.parser")
            td_list = soup.find_all("td", class_="display-bg")
            found = False
            for td in td_list:
                if td.get_text(strip=True).lower() == "sportsbest":
...
def fetch_sportsbest_m3u8_with_proxy(url):
    with sync_playwright() as p:
        ...
        page = context.new_page()
        m3u8_urls = set()
        def capture(req_or_resp):
            u = getattr(req_or_resp,'url',None)
            if u and ".m3u8" in u and "embedsports.top" in u:
                m3u8_urls.add(u)
        ...
        try:
            ...
            pass
        return [PROXY_PREFIX + u for u in m3u8_urls]

def parse_match_name(raw_name):
    name = raw_name.replace("Match Started", "").strip()
    words = re.findall(r'[A-Z][a-z0-9]*(?:\s[A-Z][a-z0-9]*)*', name)
    if len(words) >= 2:
        mid = len(words) // 2
